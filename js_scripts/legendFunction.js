function legendinfo() {
  ...
  dataSet_link = ul2.selectAll("li").data(data_Set).enter().append("li");
  ...
  datasettext_click = dataSet_link
    .append("span")
    .text((d) => d.category)
    .style("font-size", "14.208px")
    .style("font-family", "Arial")
  ...
  dataSet_disease
    .filter((d) => disease_Class_Data.includes(d.category))
    .append("div")
    .attr("class", "triangle")
    ...
    .style("border-bottom", function (d) {
      var color =
        healthCategoriesWithColors.find(
  ...
  disease_clicked = dataSet_disease
    .filter((d) => disease_Class_Data.includes(d.category))
    .append("span")
    .text((d) => d.category)
    .style("font-size", "14.208px")
...
ul_color.addEventListener("click", function (event) {
  ...
  node.each(function (node) {
    if (selected_maxphase === "Unknown" || selected_maxphase === "") {
      if (
        (node.MAX_PHASE === "Unknown" || node.MAX_PHASE === "") &&
        node.type === "parentnode"
      ) {
        d3.select(this).select("rect").attr("fill", colorpick);
      }
    } else if (
      node.MAX_PHASE === selected_maxphase &&
      node.type === "parentnode"
    ) {
      d3.select(this).select("rect").attr("fill", colorpick);
    }
...
function onclickmax_phase(event) {
  not_remove = false;
  d3.select(this).classed("marked", function () {
